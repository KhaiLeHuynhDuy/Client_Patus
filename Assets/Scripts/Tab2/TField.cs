...
public class TField2 : IActionListener2
{
	public bool isFocus;
	...
	public static TField2 currentTField;
	...
	public Command2 cmdClear;

	public Command2 cmdDoneAction;

	private mScreen2 parentScr;
	...
	public TField2(mScreen2 parentScr)
	{
		text = string.Empty;
	...
	public TField2(int x, int y, int w, int h)
	{
		text = string.Empty;
		init();
		this.x = x;
	...
	public TField2(string text, int maxLen, int inputType)
	{
		this.text = text;
	...
	public void init()
	{
		CARET_HEIGHT = mScreen2.ITEM_HEIGHT + 1;
		cmdClear = new Command2(mResources2.DELETE, this, 1000, null);
		if (Main2.isPC)
		{
			typeXpeed = 0;
		}
		if (imgTf == null)
		{
			imgTf = GameCanvas2.loadImage("/mainImage/myTexture2dtf.png");
		}
	}
	...
	{
		if (paintedText != null && mFont2.tahoma_8b != null)
		{
			...
			if (offsetX < 0 && mFont2.tahoma_8b.getWidth(paintedText) + offsetX < width - TEXT_GAP_X - 13 - typingModeAreaWidth)
			{
				offsetX = width - 10 - typingModeAreaWidth - mFont2.tahoma_8b.getWidth(paintedText);
			}
			if (offsetX + mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos)) <= 0)
			{
				offsetX = -mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos));
				offsetX += 40;
			}
			else if (offsetX + mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos)) >= width - 12 - typingModeAreaWidth)
			{
				offsetX = width - 10 - typingModeAreaWidth - mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos)) - 2 * TEXT_GAP_X;
			}
			if (offsetX > 0)
			{
				offsetX = 0;
	...
	private void keyPressedAscii(int keyCode)
	{
		...
		{
			string input;
			if (Main2.isPC && keyCode == ((int)KeyCode.V) && MainMod2.pasteText.Length > 0)
			{
				input = MainMod2.pasteText;
				MainMod2.pasteText = "";
			}
			else
			{
				input = ((char)keyCode).ToString();
			}

			string text = this.text.Substring(0, caretPos) + input;
			if (caretPos < this.text.Length)
			{
	...
	public void setOffset(int index)
	{
		...
		int num = mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos));
		switch (index)
		{
			case -1:
				if (num + offsetX < 15 && caretPos > 0 && caretPos < paintedText.Length)
				{
					offsetX += mFont2.tahoma_8b.getWidth(paintedText.Substring(caretPos, 1));
				}
				break;
			case 1:
				if (num + offsetX > width - 25 && caretPos < paintedText.Length && caretPos > 0)
				{
					offsetX -= mFont2.tahoma_8b.getWidth(paintedText.Substring(caretPos - 1, 1));
				}
				break;
			default:
		...
		{
			int num2 = mFont2.tahoma_8b.getWidth(paintedText) - (width - 12);
			if (offsetX < -num2)
			{
				offsetX = -num2;
			}
	...
	public void paintInputTf(mGraphics2 g, bool iss, int x, int y, int w, int h, int xText, int yText, string text, string info)
	{
		g.setColor(0);
		if (iss)
		{
		...
		{
			mFont2.tahoma_8b.drawString(g, text, xText, yText, 0);
		}
		else if (info != null)
		{
			if (iss)
			{
				mFont2.tahoma_7b_focus.drawString(g, info, xText, yText, 0);
			}
			else
			{
				mFont2.tahoma_7b_unfocus.drawString(g, info, xText, yText, 0);
			}
		}
	}
	...
	public void paint(mGraphics2 g)
	{
		...
		paintInputTf(g, flag, x, y - 1, width, height + 5, TEXT_GAP_X + offsetX + x + 1, y + (height - mFont2.tahoma_8b.getHeight()) / 2 + 2, paintedText, name);
		...
		if (flag && isPaintMouse && isPaintCarret)
		{
			...
			{
				g.setColor(7999781);
				g.fillRect(TEXT_GAP_X + 1 + offsetX + x + mFont2.tahoma_8b.getWidth(paintedText.Substring(0, caretPos) + "a") - CARET_WIDTH - mFont2.tahoma_8b.getWidth("a"), y + (height - CARET_HEIGHT) / 2 + 5, CARET_WIDTH, CARET_HEIGHT);
			}
			GameCanvas2.resetTrans(g);
			if (text != null && text.Length > 0 && GameCanvas2.isTouch)
			{
				g.drawImage(GameCanvas2.imgClear, x + width - 13, y + height / 2 + 3, mGraphics2.VCENTER | mGraphics2.HCENTER);
			}
		}
	}
	...
	{
		if (GameCanvas2.isPointerHoldIn(x + width - 20, y, 40, height))
		{
			clearAllText();
			isFocus = true;
		}
		else if (GameCanvas2.isPointerHoldIn(x, y, width - 20, height))
		{
			setFocusWithKb(isFocus: true);
		}
		else
	...
	public void setFocusWithKb(bool isFocus)
	{
		...
		if (Thread.CurrentThread.Name == Main2.mainThreadName && currentTField != null)
		{
			this.isFocus = true;
