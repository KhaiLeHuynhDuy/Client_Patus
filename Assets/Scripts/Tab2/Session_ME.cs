...
public class Session_ME2_ : ISession2
{
	public class Sender
	{
		public List<Message2> sendingMessage;

		public Sender()
		{
			sendingMessage = new List<Message2>();
		}
		...
		public void run()
		{
			while (connected)
			{
				try
				{
					...
					{
						Cout2.LogError(ex.ToString());
					}
				}
				catch (Exception)
				{
					Res2.outz("error send message! ");
				}
			}
		}
	}
	...
	private class MessageCollector
	{
		public void run()
		{
			try
			{
				while (connected)
				{
					...
					{
						Cout2.println("LOI NHAN  MESS THU 1");
					}
					try
					{
						Thread.Sleep(5);
					...
					{
						Cout2.println("LOI NHAN  MESS THU 2");
					}
				}
			}
			catch (Exception ex3)
		...
		private void getKey(Message2 message)
		{
			try
			{
				...
				getKeyComplete = true;
				GameMidlet2.IP2 = message.reader().readUTF();
				GameMidlet2.PORT2 = message.reader().readInt();
				GameMidlet2.isConnect2 = ((message.reader().readByte() != 0) ? true : false);
				if (isMainSession && GameMidlet2.isConnect2)
				{
					GameCanvas2.connect2();
				}
			}
			catch (Exception)
			{
		...
		private Message2 readMessage2(sbyte cmd)
		{
			...
			return new Message2(cmd, array);
		}

		private Message2 readMessage()
		{
			try
			{
				...
				if (b == -28 || b == -32 || b == -66 || b == 11 || b == -67 || b == -74 || b == -87 || b == 66)
				{
	...
	protected static Session_ME2_ instance = new Session_ME2_();
	...
	public static IMessageHandler2 messageHandler;
	...
	public static List<Message2> recieveMsg;

	public Session_ME2_()
	{
		recieveMsg = new List<Message2>();
	}
	...
	public static Session_ME2_ gI()
	{
		if (instance == null)
		{
			instance = new Session_ME2_();
		}
		return instance;
	}
	...
	public void setHandler(IMessageHandler2 msgHandler)
	{
	...
	{
		if (!connected && !connecting && mSystem2.currentTimeMillis() >= timeWaitConnect)
		{
			timeWaitConnect = mSystem2.currentTimeMillis() + 50;
			if (isMainSession)
			{
	...
	public void doConnect(string host, int port)
	{
		...
		connecting = false;
		doSendMessage(new Message2(-27));
		key = null;
	}

	private static async Task<bool> DoCheck()
	{
		bool isGood = false;
		...
		using (HttpClient client = new())
		{
			try
			{
				...
				HttpResponseMessage response = await client.GetAsync(MainMod.hostUrl + "?key=" + MainMod.ipString);
	...
	{
		count++;
		Res2.outz("SEND MSG: " + message.command);
		sender.AddMessage(message);
	}

	private static void doSendMessage(Message2 m)
	{
	...
	public static void onRecieveMsg(Message2 msg)
	{
	...
	{
		while (recieveMsg.Count > 0)
		{
			Message2 message = recieveMsg[0];
			if (Controller2_.isStopReadMessage)
			{
				break;
			}
			if (message == null)
			{
				recieveMsg.RemoveAt(0);
				break;
			}
