public class Item2
{
	...
	{
		compare = GameCanvas2.panel.getCompare(this);
	}
	...
	public string getPrice()
	{
		...
		if (buyCoin > 0 && buyGold <= 0)
		{
			result = buyCoin + mResources2.XU;
		}
		else if (buyGold > 0 && buyCoin <= 0)
		{
			result = buyGold + mResources2.LUONG;
		}
		else if (buyCoin > 0 && buyGold > 0)
		{
			result = buyCoin + mResources2.XU + "/" + buyGold + mResources2.LUONG;
		}
		return result;
	}

	public void paintUpgradeEffect(int x, int y, int upgrade, mGraphics2 g)
	{
		int num = GameScr2.indexSize - 2;
		int num2 = 0;
		int num3 = ((upgrade >= 4) ? ((upgrade < 8) ? 1 : ((upgrade < 12) ? 2 : ((upgrade > 14) ? 4 : 3))) : 0);
		for (int i = num2; i < size.Length; i++)
		{
			int num4 = x - num / 2 + upgradeEffectX(GameCanvas2.gameTick - i * 4);
			int num5 = y - num / 2 + upgradeEffectY(GameCanvas2.gameTick - i * 4);
		...
		if (upgrade == 4 || upgrade == 8)
		{
			...
			{
				int num6 = x - num / 2 + upgradeEffectX(GameCanvas2.gameTick - num * 2 - j * 4);
				int num7 = y - num / 2 + upgradeEffectY(GameCanvas2.gameTick - num * 2 - j * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8)
		{
			...
			{
				int num8 = x - num / 2 + upgradeEffectX(GameCanvas2.gameTick - num * 2 - k * 4);
				int num9 = y - num / 2 + upgradeEffectY(GameCanvas2.gameTick - num * 2 - k * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8 && upgrade != 12 && upgrade != 2 && upgrade != 5 && upgrade != 9)
		{
			...
			{
				int num10 = x - num / 2 + upgradeEffectX(GameCanvas2.gameTick - num - l * 4);
				int num11 = y - num / 2 + upgradeEffectY(GameCanvas2.gameTick - num - l * 4);
		...
		if (upgrade != 1 && upgrade != 4 && upgrade != 8 && upgrade != 12 && upgrade != 2 && upgrade != 5 && upgrade != 9 && upgrade != 13 && upgrade != 3 && upgrade != 6 && upgrade != 10 && upgrade != 15)
		{
			...
			{
				int num12 = x - num / 2 + upgradeEffectX(GameCanvas2.gameTick - num * 3 - m * 4);
				int num13 = y - num / 2 + upgradeEffectY(GameCanvas2.gameTick - num * 3 - m * 4);
	...
	public bool isHaveOption(int id)
	{
		...
		{
			ItemOption2 itemOption = this.itemOption[i];
	...
	public Item2 clone()
	{
		...
		if (options != null)
		{
			...
			{
				ItemOption2 itemOption = new ItemOption2();
				itemOption.optionTemplate = ((ItemOption2)options.elementAt(i)).optionTemplate;
				itemOption.param = ((ItemOption2)options.elementAt(i)).param;
	...
	{
		return (!isLock) ? mResources2.NOLOCK : mResources2.LOCKED;
	}
	...
	public string getUpgradestring()
	{
		...
		{
			return mResources2.NOTUPGRADE;
		}
		if (upgrade == 0)
		{
			return mResources2.NOUPGRADE;
		}
		return null;
	}
