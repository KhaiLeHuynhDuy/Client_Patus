public class Scroll2
{
	...
	public ScrollResult2 updateKey()
	{
		if (styleUPDOWN)
		{
	...
	public ScrollResult2 updateKey(bool isGetSelectNow)
	{
		if (styleUPDOWN)
		{
	...
	private ScrollResult2 updateKeyScrollUpDown(bool isGetNow)
	{
		int num = xPos;
		int num2 = yPos;
		int w = width;
		...
		{
			if (!pointerIsDowning && GameCanvas2.isPointer(num, num2, w, h))
			{
			...
			else if (pointerIsDowning)
			{
				...
				if (pointerDownTime > 5 && pointerDownFirstX == GameCanvas2.py && !isDownWhenRunning)
				{
					...
					if (ITEM_PER_LINE > 1)
					{
						int num3 = (cmtoY + GameCanvas2.py - num2) / ITEM_SIZE;
						int num4 = (cmtoX + GameCanvas2.px - num) / ITEM_SIZE;
						selectedItem = num3 * ITEM_PER_LINE + num4;
					}
					else
					{
						selectedItem = (cmtoY + GameCanvas2.py - num2) / ITEM_SIZE;
					}
				}
				int num5 = GameCanvas2.py - pointerDownLastX[0];
				if (!isGetNow)
				{
		...
		if (GameCanvas2.isPointerJustRelease && pointerIsDowning)
		{
			...
			if (Res2.abs(i2) < 20 && Res2.abs(GameCanvas2.py - pointerDownFirstX) < 20 && !isDownWhenRunning)
			{
				...
				if (ITEM_PER_LINE > 1)
				{
					int num7 = (cmtoY + GameCanvas2.py - num2) / ITEM_SIZE;
					int num8 = (cmtoX + GameCanvas2.px - num) / ITEM_SIZE;
			...
			else if ((selectedItem == -1 && !isDownWhenRunning) || (isGetNow && selectedItem != -1 && !isDownWhenRunning))
			{
				...
				{
					int num9 = GameCanvas2.py - pointerDownLastX[0] + (pointerDownLastX[0] - pointerDownLastX[1]) + (pointerDownLastX[1] - pointerDownLastX[2]);
		...
		ScrollResult2 scrollResult = new ScrollResult2();
	...
	private ScrollResult2 updateKeyScrollLeftRight()
	{
		int num = xPos;
		int y = yPos;
		int w = width;
		...
		if (GameCanvas2.isPointerDown)
		{
			if (!pointerIsDowning && GameCanvas2.isPointer(num, y, w, h))
			{
				for (int i = 0; i < pointerDownLastX.Length; i++)
				{
			...
			else if (pointerIsDowning)
			{
				...
				{
					pointerDownFirstX = -1000;
					selectedItem = (cmtoX + GameCanvas2.px - num) / ITEM_SIZE;
				}
				int num2 = GameCanvas2.px - pointerDownLastX[0];
		...
		if (GameCanvas2.isPointerJustRelease && pointerIsDowning)
		{
			...
			else if (selectedItem == -1 && !isDownWhenRunning)
			{
				...
				{
					int num4 = GameCanvas2.px - pointerDownLastX[0] + (pointerDownLastX[0] - pointerDownLastX[1]) + (pointerDownLastX[1] - pointerDownLastX[2]);
	...
	public void setStyle(int nItem, int ITEM_SIZE, int xPos, int yPos, int width, int height, bool styleUPDOWN, int ITEM_PER_LINE)
	{
		...
		Res2.outz("nItem= " + nItem + " ITEMSIZE= " + ITEM_SIZE + " heghit= " + height);
		if (styleUPDOWN)
		{
