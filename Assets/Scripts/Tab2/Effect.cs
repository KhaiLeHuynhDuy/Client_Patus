...
public class Effect2_
{
	...
	public static MyVector2 vEffData = new MyVector2();

	public int trans;

	public long timeExist;

	public static MyVector2 lastEff = new MyVector2();

	public static MyVector2 newEff = new MyVector2();
    ...
    public Effect2_(int id, Char2 c, int layer, int loop, int loopCount, sbyte isStand, int xC, int yC)
    {
        this.c = c;
        effId = id;
        this.layer = layer;
        ...
        {
            EffectData2 effectData = new EffectData2
            {
                ID = id
            };
            if (id >= 42 && id <= 46)
            ...
            string text = "/x" + mGraphics2.zoomLevel + "/effectdata/" + id + "/data";
            ...
            if (dataInputStream != null)
            {
                ...
                effectData.img = GameCanvas2.loadImage("/effectdata/" + id + "/img.png");
            }
            else
            {
                Service2.gI().getEffData((short)id);
            }
    ...
    public Effect2_(int id, Char2 c, int layer, int loop, int loopCount, sbyte isStand)
	{
		this.c = c;
		effId = id;
		this.layer = layer;
	...
	public Effect2_(int id, int x, int y, int layer, int loop, int loopCount)
	{
		this.x = x;
		this.y = y;
		effId = id;
	...
	public static void removeEffData(int id)
	{
		...
		{
			EffectData2 effectData = (EffectData2)vEffData.elementAt(i);
			if (effectData.ID == id)
			{
	...
	public static void addEffData(EffectData2 eff)
	{
		vEffData.addElement(eff);
		if (TileMap2.mapID != 130 && vEffData.size() > 10)
		{
			for (int i = 0; i < 5; i++)
			{
	...
	public static EffectData2 getEffDataById(int id)
	{
		for (int i = 0; i < vEffData.size(); i++)
		{
			EffectData2 effectData = (EffectData2)vEffData.elementAt(i);
	...
	public void paintUnderBackground(mGraphics2 g, int xLayer, int yLayer)
	{
	...
	public void paint(mGraphics2 g)
	{
		...
		try
		{
			getEffDataById(effId).paintFrame(g, currFrame, x + xC, y + yC, trans, layer);
		}
		catch (Exception)
		{
		}
			...
			else
			{
				...
				if (data != null)
				{
					if (!isGetTime)
					{
						...
						if (typeEff == 0)
						{
							t = Res2.random(indexFrom, indexTo);
						}
					}
					switch (typeEff)
					{
					...
					case 3:
						...
						break;
					case 0:
						if (Res2.inRect(x - 50, y - 50, 100, 100, Char2.myCharz().cx, Char2.myCharz().cy) && t > indexFrom && t < indexTo)
						{
							if (t < indexTo)
							{
								t = indexTo;
					...
					case 2:
						...
						{
							tLoopCount = 0;
							trans = Res2.random(0, 2);
						}
						break;
					}
					if (t == data.Length / 2 && (effId == 62 || effId == 63 || effId == 64 || effId == 65))
					{
						SoundMn2.playSound(x, y, SoundMn2.FIREWORK, SoundMn2.volume);
					}
					if (t <= data.Length - 1)
					{
						currFrame = data[t];
				...
				if (t >= data.Length - 1)
				{
					...
					if (tLoop == -1)
					{
						EffecMn2.vEff.removeElement(this);
					}
					if (typeEff == 2)
					{
						t = 0;
