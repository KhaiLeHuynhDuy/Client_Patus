...
public class DataInputStream2
{
	...
	public static DataInputStream2 istemp;

	private static int status;

	private static string filenametemp;

	public DataInputStream2(string filename)
	{
		TextAsset textAsset = (TextAsset)Resources.Load(filename, typeof(TextAsset));
		r = new myReader2(ArrayCast2.cast(textAsset.bytes));
	}

	public DataInputStream2(sbyte[] data)
	{
		r = new myReader2(data);
	}
	...
	public static DataInputStream2 getResourceAsStream(string filename)
	{
		return __getResourceAsStream(filename);
	}

	private static DataInputStream2 _getResourceAsStream(string filename)
	{
		if (status != 0)
		{
			for (int i = 0; i < 500; i++)
	...
	private static DataInputStream2 __getResourceAsStream(string filename)
	{
		try
		{
			return new DataInputStream2(filename);
		}
		catch (Exception)
		{
			return null;
