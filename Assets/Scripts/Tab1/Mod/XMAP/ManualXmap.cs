        ...
        public static void LoadMap(int position)
        {
            ...
            switch (position)
            {
                case 0:
                    ...
                    {
                        MainMod.MoveTo(wayPointMapLeft[0], wayPointMapLeft[1]);
                    }
                    else
                    {
                        MainMod.MoveTo(60, GetYGround(60));
                    }
                    break;
                case 1:
                    if (wayPointMapRight[0] != 0 && wayPointMapRight[1] != 0)
                    {
                        MainMod.MoveTo(wayPointMapRight[0], wayPointMapRight[1]);
                    }
                    else
                    {
                        MainMod.MoveTo(TileMap.pxw - 60, GetYGround(TileMap.pxw - 60));
                    }
                    break;
                case 2:
                    if (wayPointMapCenter[0] != 0 && wayPointMapCenter[1] != 0)
                    {
                        MainMod.MoveTo(wayPointMapCenter[0], wayPointMapCenter[1]);
                    ...
                    {
                        MainMod.MoveTo(TileMap.pxw / 2, GetYGround(TileMap.pxw / 2));
                    }
                    break;
            }
        }
        ...
        private static void TeleportInNRDMap(int position)
        {
            ...
            for (int i = 0; i < GameScr.vNpc.size(); i++)
            {
                ...
                if (npc.template.npcTemplateId >= 30 && npc.template.npcTemplateId <= 36)
                {
                    ...
                    MainMod.MoveTo(npc.cx, npc.cy - 3);
                    return;
                }
            }
        }
