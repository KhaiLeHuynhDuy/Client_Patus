...
namespace Mod.XMAP
{
    public class XmapController : IActionListener
    {
        ...
        {
            bool flag = IsWaiting() || XmapData.GI().IsLoading;
            if (!flag)
            {
                if (IsWaitNextMap)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (TileMap.mapID == WayXmap[^1] && !XmapData.IsMyCharDie())
                        {
                            GameScr.info1.addInfo("|4|Modded by patus", 0);
                            FinishXmap();
                        }
                        else
                        {
        ...
        private static void NextMapAutoWaypoint(MapNext mapNext)
        {
            ...
            if (waypoint != null)
            {
                ...
                MainMod.MoveTo(posWaypointX, posWaypointY);
            ...
            else
            {
                ...
                for (int i = 0; i < GameCanvas.menu.menuItems.size(); i++)
                {
                    ...
                    if (VuDangMapNext >= 161 && VuDangMapNext <= 164)
                    {
                        ...
                        {
                            Service.gI().useItem(0, 1, (sbyte)MainMod.FindItemIndex(992), -1);
                        }
                        return;
                    }
                }
        ...
        public static void UseCapsuleNormal()
        {
            ...
            Service.gI().useItem(0, 1, (sbyte)MainMod.FindItemIndex(193), -1);
        }

        public static void UseCapsuleVip()
        {
            AutoXmap.IsShowPanelMapTrans = false;
            Service.gI().useItem(0, 1, (sbyte)MainMod.FindItemIndex(194), -1);
        }
