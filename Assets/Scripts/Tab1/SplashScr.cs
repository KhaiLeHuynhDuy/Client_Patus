public class SplashScr : mScreen
{
	...
	{
		if (MainMod.imgLogoBig != null && splashScrStat < 30)
		{
			g.setColor(0);
			...
			g.drawImageScale(MainMod.imgLogoBig, (GameCanvas.w - imgW) / 2, (GameCanvas.h - imgH) / 2, imgW, imgH);
		}
		if (nData != -1)
		{
			g.setColor(0);
			...
			int imgW = MainMod.imgLogoBig.getWidth() * mGraphics.zoomLevel / 4;
			int imgH = MainMod.imgLogoBig.getHeight() * mGraphics.zoomLevel / 4;
			g.drawImageScale(MainMod.imgLogoBig, (GameCanvas.w - imgW) / 2, (GameCanvas.h - imgH) / 2 - 24, imgW, imgH);
