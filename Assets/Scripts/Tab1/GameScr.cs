...
public class GameScr : mScreen, IChatable
{
    ...
    public void readSkill()
    {
        ...
        {
            MainMod.Log("Loi ham readSkill: " + ex.ToString());
        }
        finally
        {
            try
            ...
            {
                MainMod.Log("Loi ham readskill 1: " + ex2.ToString());
            }
        }
    }
            ...
            if (GameCanvas.keyAsciiPress != 0)
            {
                if (mobCapcha == null)
                {
                    if (TField.isQwerty)
                    {
                        ...
                        else if (MainMod.GI().UpdateKey(GameCanvas.keyAsciiPress))
                        {
    ...
    public void doSelectSkill(Skill skill, bool isShortcut)
    {
        ...
        // if (Char.myCharz().myskill != skill && (Char.myCharz().dart != null || Char.myCharz().arr != null || Char.myCharz().skillInfoPaint() != null))
        // {
        //     return;
        // }
        Char.myCharz().myskill = skill;
    ...
    private void UpdateKeyTouchControl()
    {
        ...
        keyTouch = -1;
        int numY = 52;
        if (MainMod.notifBoss)
        {
            for (int i = 0; i < MainMod.bossNotif.size(); i++)
            {
                ShowBoss showBoss = (ShowBoss)MainMod.bossNotif.elementAt(i);
                if (showBoss == null)
                {
                    continue;
                }
                if (GameCanvas.isPointerHoldIn(GameCanvas.w - 5 - showBoss.stringLength, numY + 3, showBoss.stringLength, 10) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                {
                    MainMod.GI().GoToBoss(showBoss.mapID);
        ...
        if (MainMod.showCharsInMap)
        {
            MyVector chars = MainMod.GI().charsInMap;
            for (int i = 0; i < chars.size(); i++)
            {
                ...
                if (@char != null)
                {
                    if (GameCanvas.isPointerHoldIn(GameCanvas.w - 130, numY, 130, 10) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                    {
                        ...
                        {
                            MainMod.MoveTo(@char.cx, @char.cy);
                        }
                        else
                        {
    ...
    public override void update()
    {
        ...
        if (!PickMob.tanSat && isAutoPlay && GameCanvas.gameTick % 20 == 0)
        {
            autoPlay();
        }
        updateXoSo();
    ...
    public void onChatFromMe(string text, string to)
    {
        ...
        {
            MainMod.GI().ChangeGameSpeed(text);
        ...
        {
            MainMod.GI().SetIncreasePoint(text);
            ChatTextField.gI().strChat = "Chat";
            ChatTextField.gI().tfChat.name = "chat";
            ChatTextField.gI().tfChat.setIputType(TField.INPUT_TYPE_ANY);
            ChatTextField.gI().isShow = false;
        }
        else if (ChatTextField.gI().strChat == "% HP" && text != string.Empty)
        {
            MainMod.SetPercentBean(text, 0);
            ChatTextField.gI().strChat = "Chat";
            ChatTextField.gI().tfChat.name = "chat";
            ChatTextField.gI().tfChat.setIputType(TField.INPUT_TYPE_ANY);
            ChatTextField.gI().isShow = false;
        }
        else if (ChatTextField.gI().strChat == "% MP" && text != string.Empty)
        {
            MainMod.SetPercentBean(text, 1);
            ChatTextField.gI().strChat = "Chat";
            ChatTextField.gI().tfChat.name = "chat";
            ChatTextField.gI().tfChat.setIputType(TField.INPUT_TYPE_ANY);
            ChatTextField.gI().isShow = false;
    ...
    public void openUIZone(Message message)
    {
        ...
        if (MainMod.GI().userOpenZones)
        {
            GameCanvas.panel.setTypeZone();
            GameCanvas.panel.show();
            MainMod.GI().userOpenZones = false;
        }
    }
    ...
    private void paintGamePad(mGraphics g)
    {
        if (isAnalog != 0 && Char.myCharz().statusMe != 14)
        {
            ...
            MainMod.GI().PaintButton(g, xTG, yTG);
        }
    }
    ...
    public void chatVip(string chatVip)
    {
        ...
        {
            MainMod.bossNotif.addElement(new ShowBoss(chatVip));
            if (MainMod.bossNotif.size() > 5)
            {
                MainMod.bossNotif.removeElementAt(0);
            }
        }
    }
