			...
			try
			{
				switch (msg.command)
				{
					...
					case 102:
						{
							...
							if (action == 0 || action == 1 || action == 2 || action == 6)
							{
								...
								sbyte charSz = msg.reader().readByte();
								Char[] charList = new Char[charSz];
								int[] dameList = new int[charSz];
								for (int num28 = 0; num28 < charSz; num28++)
								{
									int num29 = msg.reader().readInt();
									charList[num28] = null;
									if (num29 != Char.myCharz().charID)
									{
										charList[num28] = GameScr.findCharInMap(num29);
									}
									else
									{
										charList[num28] = Char.myCharz();
									}
									dameList[num28] = msg.reader().readInt();
								}
								bigBoss.setAttack(charList, dameList, action);
							}
							if (action == 3 || action == 4 || action == 5 || action == 7)
							{
								...
								if (action == 3 || action == 4)
								{
									...
									bachTuoc.setAttack(array6, array7, action);
								}
								if (action == 5)
								{
							...
							if (action > 9 && action < 30)
							{
								readActionBoss(msg, action);
							}
							break;
						}
					case 101:
					...
					case 98:
						{
							string str = msg.reader().readUTF();
							MainMod.GI().AddNotifTichXanh(str);
							break;
						}
					case -106:
						{
		...
		private static void readInfoEffChar(Message msg)
		{
			try
			{
				...
				if (b == 0) // bat
				{
					...
					@char?.addEffChar(new Effect(id, @char, layer, loop, loopCount, isStand, xC, yC));
				}
				else if (b == 1) // tat
				{
					int id2 = msg.reader().readShort();
