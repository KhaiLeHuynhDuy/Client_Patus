...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    private void initCommand()
    {
        ...
        cmd = new Command[3 + nCmdPlay + (MainMod.isDebug ? 1 : 0)];
            ...
            switch (i)
            {
                ...
                case 3:
                    ...
                    else
                    {
                        if (MainMod.isDebug)
                        {
                            cmd[3] = new Command(MainMod.ipServer, MainMod.GI(), 8, text + "|" + text2);
                        }
                    }
                    break;
                case 4:
                    cmd[4] = new Command(MainMod.ipServer, MainMod.GI(), 8, text + "|" + text2);
                    break;
            }
            cmd[i].y = num;
            cmd[i].setType();
    ...
    public override void paint(mGraphics g)
    {
        ...
        if (GameCanvas.currentDialog == null)
        {
            if (!loadScreen)
            {
                if (!bigOk)
                {
                    g.drawImageScale(MainMod.imgBg, 0, 0, GameCanvas.w, GameCanvas.h, 0);
                    ...
                    g.drawImageScale(MainMod.imgLogoBig, GameCanvas.hw - (imgW / 2), GameCanvas.hh - (imgH / 2) - 32, imgW, imgH);
            ...
            else
            {
                ...
                if (MainMod.imgLogoBig != null)
                {
                    //g.drawImage(SplashScr.imgLogo, GameCanvas.hw, num4, 3);

                    int imgW = MainMod.imgLogoBig.getWidth() * mGraphics.zoomLevel / 4;
                    int imgH = MainMod.imgLogoBig.getHeight() * mGraphics.zoomLevel / 4;
                    g.drawImageScale(MainMod.imgLogoBig, GameCanvas.hw - (imgW / 2), num4 - (imgH / 2), imgW, imgH);
                }
                for (int i = 0; i < cmd.Length; i++)
                {
                    cmd[i].paint(g);
    ...
    {
        flagServer = MainMod.isAutoLogin ? 120 : 30;
    ...
    public override void update()
    {
        ...
        if (MainMod.isAutoLogin || countDieConnect < 5)
        {
            if (flagServer <= 0)
            {
                flagServer = MainMod.isAutoLogin ? 120 : 30;
    ...
    public static void SaveIP()
    {
        try
        {
            Rms.saveRMSString("NRlink2", MainMod.EncodeStringToByteArrayString(linkDefault, "6"));
            SplashScr.loadIP();
        }
        catch (Exception)
        {
    ...
    public static void SaveIPNew(string ip)
    {
        try
        {
            Rms.saveRMSString("NRlink2", MainMod.EncodeStringToByteArrayString(ip, "6"));
            SplashScr.loadIP();
        }
        catch (Exception)
        {
    ...
    public static void LoadIP()
    {
        ...
        string ip = MainMod.DecodeByteArrayString(load, "6");
