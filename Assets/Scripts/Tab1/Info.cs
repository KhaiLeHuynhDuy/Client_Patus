	...
	public void paint(mGraphics g, int x, int y, int dir)
	{
		...
		if (says != null && says.Length != 0 && type != 1)
		{
			...
			for (int i = 0; i < says.Length; i++)
			{
				...
				info.charInfo.paintCharBody(g, X - 2, Y + H + 2, 1, 0, true);
				if (mGraphics.zoomLevel == 1)
				{
					...
					{
						MainMod.PaintTicks(g, false, X + 8, Y + 2);
					}
					else if (info.charInfo.isTichXanh)
					{
						MainMod.PaintTicks(g, true, X + 8, Y + 2);
					}
				}
				else
				{
					...
					if (info.charInfo.isTichDo)
					{
						MainMod.PaintTicks(g, false, X + 9, Y - 2);
					}
					else if (info.charInfo.isTichXanh)
					{
						MainMod.PaintTicks(g, true, X + 9, Y - 2);
					}
				}
