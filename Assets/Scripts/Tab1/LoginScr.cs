...
public class LoginScr : mScreen, IActionListener
{
    ...
    public LoginScr()
    {
        ...
        cmdOK.y = cmdLogin.y;
        MainMod.cmdAccManager = new(MainMod.strAccManager, MainMod.GI(), 101, null);
        MainMod.cmdAccManager.x = GameCanvas.w / 2 + 3;
        MainMod.cmdAccManager.y = cmdLogin.y;

        center = cmdOK;
        left = MainMod.cmdAccManager;

        MainMod.cmdCloseAccManager = new(mResources.CLOSE, MainMod.GI(), 104, null);
    ...
    public override void paint(mGraphics g)
    {
        ...
        if (GameCanvas.currentDialog == null)
        {
            if (MainMod.isOpenAccMAnager)
            {
                ...
                for (int i = 0; i < MainMod.GI().cmdsChooseAcc.Count; i++)
                {
                    MainMod.GI().cmdsChooseAcc[i].x = xPop + 20;
                    MainMod.GI().cmdsChooseAcc[i].y = yStart;
                    MainMod.GI().cmdsChooseAcc[i].paint(g);

                    MainMod.GI().cmdsDelAcc[i].x = xPop + 190;
                    MainMod.GI().cmdsDelAcc[i].y = yStart;
                    MainMod.GI().cmdsDelAcc[i].paint(g);

                    yStart += 30;
                }

                MainMod.cmdCloseAccManager.x = (GameCanvas.w - MainMod.cmdCloseAccManager.w) / 2;
                MainMod.cmdCloseAccManager.y = yPop + h + 10;
                MainMod.cmdCloseAccManager.paint(g);

                if (GameCanvas.h > 160 && MainMod.imgLogoBig != null)
                {
                    //g.drawImage(SplashScr.imgLogo, GameCanvas.hw, yPop - 20, 3);

                    int imgW = MainMod.imgLogoBig.getWidth() * mGraphics.zoomLevel / 4;
                    int imgH = MainMod.imgLogoBig.getHeight() * mGraphics.zoomLevel / 4;
                    g.drawImageScale(MainMod.imgLogoBig, GameCanvas.hw - (imgW / 2), yPop - (imgH / 2) - 20, imgW, imgH);
                }
            }
            else
            {
                ...
                if (GameCanvas.h > 160 && MainMod.imgLogoBig != null)
                {
                    ...
                    int imgW = MainMod.imgLogoBig.getWidth() * mGraphics.zoomLevel / 4;
                    int imgH = MainMod.imgLogoBig.getHeight() * mGraphics.zoomLevel / 4;
                    g.drawImageScale(MainMod.imgLogoBig, GameCanvas.hw - (imgW / 2), num - (imgH / 2) - 15, imgW, imgH);
                }
                int num2 = 4;
                int num3 = num2 * 32 + 23 + 33;
                if (num3 >= GameCanvas.w)
    ...
    public override void updateKey()
    {
        ...
        if (GameCanvas.isPointerJustRelease && (!isLogin2 || isRes) && !MainMod.isOpenAccMAnager)
        {
        ...
        if (MainMod.isOpenAccMAnager)
        {
            foreach (Command cmd in MainMod.GI().cmdsChooseAcc)
            {
            ...
            foreach (Command cmd in MainMod.GI().cmdsDelAcc)
            {
                if (cmd.isPointerPressInside())
                {
                    cmd.performAction();
            ...
            if (MainMod.cmdCloseAccManager.isPointerPressInside())
            {
                MainMod.cmdCloseAccManager.performAction();
            }
        }
        base.updateKey();
        GameCanvas.clearKeyPressed();
    ...
    public void perform(int idAction, object p)
    {
        switch (idAction)
        {
            ...
            case 2008:
                ...
                MainMod.GI().AddAccount(user, pass);
                Rms.saveRMSString("acc", user);
