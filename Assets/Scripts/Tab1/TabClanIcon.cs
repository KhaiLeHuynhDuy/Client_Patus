public class TabClanIcon : IActionListener
{
    ...
    public void paintIcon(mGraphics g)
    {
        ...
        PopUp.paintPopUp(g, x + w + 10, y, w - 20, h, -1, isButton: true);
        mFont.tahoma_7b_dark.drawString(g, mResources.select_clan_icon, x + w / 2, y - 7, 2);
        mFont.tahoma_7b_dark.drawString(g, "Chỉ số tăng thêm", x + w + 10 + ((w - 20) / 2), y + 10, 2);
        if (lastSelect >= 0 && lastSelect <= ClanImage.vClanImage.size() - 1)
        {
            ...
            // if (clanImage.options == null || clanImage.options.Length == 0)
            // {
            //     mFont.tahoma_7_grey.drawString(g, "Không có chỉ số", x + w + 20, y + 45, 0);
            // }
            // else
            // {
            //     for (int i = 0; i < clanImage.options.Length; i++)
            //     {
            //         mFont.tahoma_7_green.drawString(g, clanImage.options[i].getOptionString(), x + w + 30, y + 25 + i * 12, 0);
            //     }
            // }
        }
