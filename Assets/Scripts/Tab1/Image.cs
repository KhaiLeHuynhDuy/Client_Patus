...
public class Image
{
	...
	public static Image CreateImageResize(string filename)
	{
		Image image = new();
		Texture2D texture2D = Resources.Load(filename) as Texture2D;
		if (texture2D == null)
		...
		if (mGraphics.zoomLevel != 4 && !filename.Contains("/tx1/"))
		{
			int targetWidth = texture2D.width / 4 * mGraphics.zoomLevel;
			int targetHeight = texture2D.height / 4 * mGraphics.zoomLevel;
