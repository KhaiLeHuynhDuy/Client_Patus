...
public class GameCanvas : IActionListener
{
    ...
    public GameCanvas()
    {
        string version = Rms.loadRMSString("GameVersion");
        if (version == null || !version.Equals(GameMidlet.VERSION))
        {
            Main.main.doClearRMS(true);
        }
        else
        {
            Main.main.doClearRMS(false);
        }
        initGame();
    }
            ...
            if (currentScreen != null)
            {
                ...
                else if (panel.isShow)
                {
                    ...
                    if (isPointer(panel.X + panel.W, panel.Y, w - panel.W * 2, panel.H)
                        ...
                        && panel.isDoneCombine)
                    {
                        if (MainMod.is2Tabs && isPointer((w - MainMod.logoW) / 2, 5, MainMod.logoW, MainMod.logoH))
                        {
                            MainMod.GI().OpenMenu();
    ...
    public static void paintBGGameScr(mGraphics g)
    {
        ...
        catch (Exception)
        {
            g.setColor(0);
            g.fillRect(0, 0, w, h);
            g.drawImageScale(MainMod.imgBg, 0, 0, w, h, 0);
        }
    }
    ...
    public void paintChangeMap(mGraphics g)
    {
        ...
        g.drawImageScale(MainMod.imgLogoBig, (w - imgW) / 2, (h - imgH) / 2 - 30, imgW, imgH);
    ...
    public static Image loadImageRMS(string path)
    {
        ...
        catch (Exception ex)
        {
            try
            {
                ...
                if (array2 != null)
                {
                    sbyte[] newArr = MainMod.DecryptBytes(array2);
                    result = Image.createImage(newArr, 0, newArr.Length);
                    array2 = null;
                    newArr = null;
                }
            }
    ...
    public static Image loadImage(string path)
    {
        ...
        try
        {
            result = Image.CreateImageResize(path);
        }
        catch (Exception)
        {
        }
